...
async def get_portfolio_service(
    repo: PortfolioRepositoryPostgres = Depends(get_portfolio_repo),
) -> PortfolioService:
    return PortfolioService(repo=repo)



async def get_trade_service(repo: TradeRepositoryPostgres = Depends(get_trade_repo)) -> TradeService:
    return TradeService(repo=repo)



async def get_user_service(repo: UserRepositoryPostgres = Depends(get_user_repo)) -> UserService:
