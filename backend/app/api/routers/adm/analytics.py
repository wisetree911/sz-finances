...
async def get_portfolio_sectors_distribution(portfolio_id: int, service: AnalyticsService=Depends(get_analytics_service))-> SectorDistributionResponse:
    return await service.sector_distribution(portfolio_id=portfolio_id)

@router.get("/{portfolio_id}/dynamics")
async def get_portfolio_dynamics(portfolio_id: int, service: AnalyticsService=Depends(get_analytics_service)) -> PortfolioDynamicsResponse:
